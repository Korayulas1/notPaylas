	...
	<main>
		...
		<section class="py-5 text-center container">
			<div class="row ">
				<div class="col-lg-6 col-md-8 mx-auto">
					<h1 class="fw-light">Ders Blog'a Hoş Geldiniz!</h1>
					<p class="lead text-muted">Burada İstediğiniz Ders ile İlgili Ders Notarına Ulaşabilirsiniz</p>
				</div>
			</div>
		</section>
		...
		<div class="album py-5 bg-light">
			<div class="container">
				...
				<div class="form-group col-md-2 ml-auto mb-3">
					<label for="kategori_adi">Kategoriler</label>
					<select name="kategori_adi" id="kategori_adi" class="form-control" onChange="window.location.href=this.value">
						<option value="">Kategori Seçin</option>
						<?php if ($categories->rowCount()): ?>
							<?php foreach ($categories as $category): ?>
								<option <?= isset($_GET['kategori']) && $_GET['kategori'] == $category['kategori_adi'] ? 'selected' : '' ?> value="index.php?kategori=<?= $category['kategori_adi'] ?>"><?= $category['kategori_adi'] ?></option>
							<?php endforeach ?>
						<?php endif ?>
				...
				<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
					...
					if (isset($_GET['kategori'])) {
						$query = $db->query("SELECT * FROM notlar WHERE kategori_adi = '{$_GET['kategori']}'", PDO::FETCH_ASSOC);
					}
					else{
					...
					if ( $query->rowCount() ){
							...
							<div class="col">
								<div class="card shadow-sm">
									...
									<div class="card-body">
										...
										<p class="card-text"><?=substr($row['not_icerik'], 0,126).'...'?></p>
										<div class="d-flex justify-content-between align-items-center">
											<div class="btn-group">
												<a href="sayfalar/detay.php?id=<?=$row['not_id']?>"><button type="button" class="btn btn-sm btn-outline-secondary">Görüntüle</button></a>
											</div>
										</div>
									</div>
								</div>
						...
						<div class="col-6 mx-auto p-3">
							<p>Bu kategoriye ait bir ders notu bulunamadı</p>
						</div>
